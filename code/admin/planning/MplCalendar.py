"""This class was copied and modified from https://github.com/meta4/mplcal"""
...
class MplCalendar(object):
    ...
    def add_event(self, day, event_str, color="black"):
        'Add an event string for the specified day'
        week, w_day = self._monthday_to_index(day)
        self.events[week][w_day][event_str] = color
    ...
    def _render(self, **kwargs):
        ...
        for week, ax_row in enumerate(axs):
            for week_day, ax in enumerate(ax_row):
                ...
                """This part is modified from the original repo"""
                events_dict = self.events[week][week_day]
                i = 0
                for elem in events_dict.keys():
                    color = events_dict[elem]
                    ax.text(.03, i + .85, elem,
                        verticalalignment='top',
                        horizontalalignment='left',
                        fontsize=9,
                        color=color)
