...
def test_inp_letter_correctly_declared():
    """Part 1. `input_letter` - function is correctly declared (name, parameter types, return type)."""
    module = reimport_module(MODULE)
...
def test_input_letter(capsys,monkeypatch):
    """Part 1. `input_letter` - Should prompt user with: Enter a single character"""
    words: list[str] = ["a"]
    set_stdin(monkeypatch, words)
    module = reimport_module(MODULE)
    module.input_letter()
...
def test_input_letter_correct(capsys,monkeypatch):
    """Part 1. `input_letter` - If the input letter has a length of 1, it should return that input letter."""
    words: list[str] = ["a"]
    set_stdin(monkeypatch, words)
    module = reimport_module(MODULE)
