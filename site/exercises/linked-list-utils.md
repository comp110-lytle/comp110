...
Recall in the past two lectures, we explored the `Node` class and wrote a recursive function that took in a `Node` object. In this exercise, you will similarly be creating functions that take in and/or return `Node` objects. Create a new folder in your `exercises` folder called `ex08`. Then, copy your code from the lecture and paste it into your `ex08` folder. You must use **recursive function calls** to implement the functions below. If you use loops, your work for that function will be disqualified.
