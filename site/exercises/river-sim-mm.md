...
## 1. Create Bear Class
All of the work for this part will be done in the `ex07` directory in the file `bear.py`.
...
### 3.3 `River#__str__()` method

Now, you are going to write your first magic method for this assignment! This will let you view the status of your river in terms of its Fish and Bear population. 

In your `River` class, modify the `__str__()` method.

#### Define 

`__str__()` should behave such that: 

- It takes no parameters other than `self` and returns `str`.
...
(Hint: You can use the special escape sequence `\n` to get a new line in a string.)

#### Call the method 

At the bottom of `river_simulation.py`, try viewing your river by calling `print(my_river)`. 
...
*Implementation hint:* Consider the fact that removing things from a list *while* you're looping through it could cause an infinite loop! Take this into account when implementing this method.
...
## 4. Magic Methods
Now we are going to use some operator overloads to add some extra functionality to our `River` class!

### 4.1 `River#__add__`

We are going to write an addition magic method (aka `__add__`) that combines two `River` objects to create one larger `River`. Modify the `__add__` method so that it sums the `Fish` and sums the `Bear`s in the two river parameters (`self` and `r`) and returns a new `River` object with those total amounts.


### 4.2 `River#__mul__`

We are going to write a multiplication magic method (aka `__mul__`) that creates larger rivers! Modify the `__mul__` method so that it returns a new `River` object with `factor` times as many `Fish` and `factor` times as many `Bear`s as the input `River` (`self`). 
